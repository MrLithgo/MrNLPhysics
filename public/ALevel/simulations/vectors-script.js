class VectorSimulator {
  ...
  resizeCanvases() {
    ...
    const sharedWidth = Math.min(
      leftRect.width || MAX_CANVAS,
      rightRect.width || MAX_CANVAS,
      MAX_CANVAS
    )
    ...
    this.leftVectors.forEach((v) => {
      if (v.start && v.end) {
        ...
        if (
          Math.abs(v.start.x - prevLeftCenter.x) < 3 &&
          Math.abs(v.start.y - prevLeftCenter.y) < 3
        ) {
          ...
          v.end = {
            x: v.start.x + length * Math.sin(radians),
            y: v.start.y - length * Math.cos(radians),
          }
        } else {
          v.start.x *= scaleLeft
          v.start.y *= scaleLeft
    ...
    this.rightVectors.forEach((v) => {
      v.start.x *= scaleRight
      v.start.y *= scaleRight
      v.end.x *= scaleRight
      v.end.y *= scaleRight
    })

    // update cached sizes and leftCenter
    this.leftClientW = clientW
    this.leftClientH = clientH
    this.rightClientW = clientW
    this.rightClientH = clientH
  ...
  setupEventListeners() {
    ...
    this.leftCanvas.addEventListener('wheel', (e) => this.handleWheel(e, this.leftCanvas), {
      passive: false,
    })
    this.rightCanvas.addEventListener('wheel', (e) => this.handleWheel(e, this.rightCanvas), {
      passive: false,
    })
  ...
  handleLeftPointerMove(e) {
    ...
    this.leftCtx.save()
    const dpr = window.devicePixelRatio || 1
    this.leftCtx.setTransform(
      dpr * this.leftScale,
      0,
      0,
      dpr * this.leftScale,
      this.leftOffset.x * dpr,
      this.leftOffset.y * dpr
    )
  ...
  handleRightPointerMove(e) {
    ...
    if (this.draggingMode === 'editEnd') {
      ...
      const linkedLeft =
        leftIndex >= 0 && this.leftVectors[leftIndex] ? this.leftVectors[leftIndex] : null
  ...
  handlePointerTrackDown(e, canvas) {
    ...
    if (pts.length === 2) {
      ...
      const offset = isLeft ? this.leftOffset : this.rightOffset
      const centerWorld = {
        x: (cssX - offset.x) / (scale || 1),
        y: (cssY - offset.y) / (scale || 1),
      }
      this._pinch = {
        canvas,
        startDist,
  ...
  handlePointerTrackMove(e, canvas) {
    ...
    if (this._pinch && this._pinch.canvas === canvas) {
      ...
      const newScale = Math.max(
        this.MIN_SCALE,
        Math.min(this._pinch.startScale * scaleFactor, this.MAX_SCALE)
      )
  ...
  handlePointerTrackUp(e, canvas) {
    ...
    Object.values(this._pointers).forEach((q) => {
      q.prevX = q.x
      q.prevY = q.y
  ...
  handlePanPointerMove(e, canvas) {
    ...
    this.panStart = { x: e.clientX, y: e.clientY }
    if (this.panTarget === 'left') {
      this.leftOffset.x += dx
      this.leftOffset.y += dy
    } else {
      this.rightOffset.x += dx
      this.rightOffset.y += dy
    }
    this.draw()
  }
  ...
  _panInertiaLoop() {
    ...
    const step = (t) => {
      ...
      if (this._panInertia.target === 'left') {
        this.leftOffset.x += dx
        this.leftOffset.y += dy
      } else {
        this.rightOffset.x += dx
      ...
      this._panInertia.vx *= decayFactor
      this._panInertia.vy *= decayFactor
      if (Math.hypot(this._panInertia.vx, this._panInertia.vy) < 10) this._panInertia.active = false
      this.draw()
      if (this._panInertia.active) requestAnimationFrame(step)
  ...
  animateZoomTo(side, targetScale, duration = 300) {
    ...
    const step = (now) => {
      ...
      if (isLeft) {
        this.leftScale = curScale
        this.leftOffset = { x: newOffsetX, y: newOffsetY }
      } else {
        this.rightScale = curScale
        this.rightOffset = { x: newOffsetX, y: newOffsetY }
      }
      this.draw()
  ...
  animateResetView(side, duration = 250) {
    ...
    const step = (now) => {
      ...
      if (isLeft) {
        this.leftScale = curScale
        this.leftOffset = curOffset
      } else {
        this.rightScale = curScale
        this.rightOffset = curOffset
      }
      this.draw()
      if (t < 1) requestAnimationFrame(step)
    }
  ...
  drawVector(ctx, start, end, color, width = 2, temporary = false, isResultant = false) {
    ...
    ctx.fillStyle = color
    ctx.lineWidth = width
    if (isResultant) ctx.setLineDash([8, 4])
    ...
    if (!temporary) {
      ...
      const label = isResultant
        ? `R=${magnitude.toFixed(1)} ${this.unitConfig[this.units].symbol}`
        : `${magnitude.toFixed(1)} ${this.unitConfig[this.units].symbol}`
  ...
  drawComponents(ctx, vector) {
    ...
    ctx.fillText(
      `${hComp} ${this.unitConfig[this.units].symbol}`,
      vector.start.x + dx / 2,
      vector.start.y - 5
    )
    ctx.fillText(
      `${vComp} ${this.unitConfig[this.units].symbol}`,
      vector.start.x + dx + 5,
      vector.start.y + dy / 2
    )
  }
  ...
  drawLeft() {
    ...
    this.leftVectors.forEach((vec, index) => {
      if (
        vec.start &&
        vec.end &&
        (typeof vec.magnitude === 'undefined' || typeof vec.angle === 'undefined')
      ) {
  ...
  drawRight() {
    ...
    this.rightCtx.save()
    const dpr = window.devicePixelRatio || 1
    this.rightCtx.setTransform(
      dpr * this.rightScale,
      0,
      0,
      dpr * this.rightScale,
      this.rightOffset.x * dpr,
      this.rightOffset.y * dpr
    )
  ...
  drawGrid(ctx) {
    ...
    let scale = 1,
      ...
      offsetY = 0
    if (ctx === this.leftCtx) {
      scale = this.leftScale || 1
      offsetX = this.leftOffset.x || 0
      offsetY = this.leftOffset.y || 0
    } else if (ctx === this.rightCtx) {
      scale = this.rightScale || 1
      offsetX = this.rightOffset.x || 0
      offsetY = this.rightOffset.y || 0
    }
         ...
         <h5 style="color: ${vector.color}">${this.unitConfig[this.units].name} ${index + 1}</h5
          <div class="vector-controls">
            <div class="input-group">
              ...
              <input type="number" id="mag_${index}" value="${
        vector.magnitude
      }" step="0.1" min="0.1" max="200.0" onchange="simulator.updateVector(${index})">
              <span>${this.unitConfig[this.units].symbol}</span>
            </div>
            <div class="input-group">
              <label>Angle:</label>
              <input type="number" id="ang_${index}" value="${(vector.angle || 0).toFixed(
        1
      )}" step="0.1" min="-360.0" max="360.0" onchange="simulator.updateVector(${index})">
              <span>°</span>
            </div>
         ...
         ${
           this.showComponents
             ? `<div class="components-display">${
                 this.unitConfig[this.units].short
               }<sub>x</sub> = ${(
                 vector.magnitude * Math.sin(((vector.angle || 0) * Math.PI) / 180)
               ).toFixed(1)} ${this.unitConfig[this.units].symbol}<br>${
                 this.unitConfig[this.units].short
               }<sub>y</sub> = ${(
                 -vector.magnitude * Math.cos(((vector.angle || 0) * Math.PI) / 180)
               ).toFixed(1)} ${this.unitConfig[this.units].symbol}</div>`
             : ''
         }
        </div>`
    })
  ...
  updateResultant() {
    if (this.rightVectors.length < 2) {
      ...
      <strong>Components:</strong> Rx = 0 ${this.unitConfig[this.units].symbol}, Ry = 0 ${
        this.unitConfig[this.units].symbol
      }
    `
      return
    }
    ...
    if (rt)
      rt.innerHTML = `<strong>Magnitude:</strong> ${magnitude.toFixed(1)} ${
        this.unitConfig[this.units].symbol
      }<br><strong>Direction:</strong> ${angle.toFixed(
        1
      )}°<br><strong>Components:</strong> R<sub>x</sub> = ${(sumX / 3).toFixed(1)} ${
        this.unitConfig[this.units].symbol
      }, R<sub>y</sub> = ${(-sumY / 3).toFixed(1)} ${this.unitConfig[this.units].symbol}`
  }
  toggleComponents() {
  ...
  loadScenario(scenario) {
    ...
    const scenarios = {
      resultant: {
        ...
        description:
          'A ship is being pulled by two tug-boats. One pulls with a force of 75 kN at an angle of 55° to the direction of travel of the ship. The other pulls with a force of 107 kN at an angle of 35° to the other side of the direction of travel. Draw a vector triangle and determine the resultant force.',
      },
      incline: {
        vectors: [
        ...
        description:
          'A box with weight, W, is at rest on an inclined plane. The normal reaction force is 65 N and the frictional force is 29 N up the slope. What is the angle of the plane?',
      },
      tension: {
        vectors: [
        ...
        description:
          'A circus performer is standing on a high wire. Their weight is 750N. One side of the wire has tension T1 and is at an angle of 30° to the horizontal and the other side has a tension of T2 and is at an agle of 45° to the horizontal. Find T1 and T2. The force diagram is drawn to a scale. Lock the angles of the tension forces and adjust the magnitude until the correct values are found.',
      },
      projectile: {
        vectors: [
  ...
  deleteVector(index) {
    ...
    this.rightVectors.forEach((v) => {
      if (typeof v.originalIndex === 'number' && v.originalIndex > index) v.originalIndex--
    })
    this.draw()
    this.updateVectorInfo()
  }
  ...
  clearAll() {
    ...
    if (compBtn) {
      ...
      compBtn.textContent = 'Show Components'
    }
    const mag = document.getElementById('magnitudeInput')
    const ang = document.getElementById('angleInput')
    if (mag) mag.value = ''
    if (ang) ang.value = ''
