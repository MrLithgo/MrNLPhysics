class RampSimulation {
  ...
  updateSimulation() {
    ...
    const gate1YPixels =
      25 +
      rampPixelPosition -
      (gate1Distance / this.rampLength) * (this.height * (this.rampWidth / this.rampLength))

    const gate2XPixels = this.rampLeft + (gate2Distance / this.rampLength) * this.rampWidth
    const gate2YPixels =
      25 +
      rampPixelPosition -
      (gate2Distance / this.rampLength) * (this.height * (this.rampWidth / this.rampLength))
  ...
  animateCar() {
    ...
    const animate = () => {
      ...
      const carYPixels =
        ...
        (currentDistance / this.rampLength) * (this.height * (this.rampWidth / this.rampLength))
  ...
  stopTimer() {
    ...
    this.status.textContent = `Trial ${
      this.trialsInCurrentSet + 1
    }/3 complete! Time: ${this.lastRecordedTime.toFixed(3)}s`
