...
// Remove mass function
function removeMass(massId) {
  const massIndex = masses.findIndex(m => m.id === massId);
  if (massIndex === -1) return;
  
  const mass = masses[massIndex];
  const element = mass.element;
  
  // Add removal animation
  element.style.transition = 'transform 0.3s ease, opacity 0.3s ease';
  element.style.transform = element.style.transform ? `${element.style.transform} scale(0.5)` : 'scale(0.5)';
...
function createMassElement(value) {
  ...
  // Create remove button with inline styles
  const removeBtn = document.createElement('button');
  removeBtn.className = 'mass-remove-btn';
  removeBtn.innerHTML = '×';
  removeBtn.type = 'button';
  removeBtn.setAttribute('aria-label', 'Remove mass');
  ...
  removeBtn.addEventListener('click', function(e) {
    ...
    if (massContainer.dataset.id) {
      const massId = parseInt(massContainer.dataset.id, 10);
      removeMass(massId);
    }
  });

  // Show/hide remove button on hover
  massContainer.addEventListener('mouseenter', () => {
    removeBtn.style.opacity = '1';
  });
  
  massContainer.addEventListener('mouseleave', () => {
  ...
  massContainer.appendChild(removeBtn);
...
function startDrag(e) {
  ...
  if (e.target.classList.contains('mass-remove-btn')) {
    return;
  }
  ...
  try {
    draggedMass.setPointerCapture(activePointerId);
  } catch (_) {}
  ...
  if (canSnap) {
    ...
    if (!advancedMode) {
      ...
      if (targetSide !== 'center') {
        const existingCount = countOnSide(targetSide, isNew ? null : massObj ? massObj.id : null);
        if (existingCount >= 1) {
          // Reject the drop
          if (isNew) {
            node.remove();
          ...
          requestAnimationFrame(() => {
            if (node && node.style) node.style.visibility = 'visible';
          });
...
function finishCleanup(node) {
  try {
    if (activePointerId !== null) node.releasePointerCapture(activePointerId);
  } catch (_) {}
...
document.getElementById('clearResultsBtn').addEventListener('click', function () {
  if (confirm('Are you sure you want to clear all results from the table?')) {
    leftTotalMass.value = '0';
    rightTotalMass.value = '0';
    leftDistance.value = '0';
    rightDistance.value = '0';
    leftMoment.value = '';
    rightMoment.value = '';

    showToast('All results cleared from table');
  }
});
