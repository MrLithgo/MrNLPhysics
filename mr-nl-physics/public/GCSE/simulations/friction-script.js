...
if (
    !document
        .getElementById("initializationGuard")
    ...
    document.addEventListener("DOMContentLoaded", function () {
        ...
        (function () {
            ...
            const pdfUrl = 'assets/worksheet.pdf';
            const fallbackFilename = 'worksheet.pdf';
        ...
        function computeInitialPositions() {
            ...
            meterInitialX = Math.min(
                meterInitialX,
                areaWidth - newtonMeter.offsetWidth - EDGE_PADDING
            );
            blockInitialX = Math.min(
                blockInitialX,
                areaWidth - block.offsetWidth - EDGE_PADDING
            );
        ...
        function clampX(x, element) {
            ...
            const maxX = Math.max(
                minX,
                experimentArea.clientWidth - element.offsetWidth - EDGE_PADDING
            );
        ...
        // Initialize
        createMeterTicks();
        initPositions(); // compute and set sensible initial positions
        updateSurface("wood");
        updateMeterReading(0);

        // Event listeners - with checks to prevent multiple attachments
        if (!newtonMeter.hasListener) {
            newtonMeter.addEventListener("mousedown", startDrag);
            newtonMeter.hasListener = true;
        }
        ...
        // Button event listeners with execution guards
        ...
        document.querySelector('[data-surface="wood"]').classList.add("active");
        ...
        window.addEventListener("resize", function () {
            ...
            const currentMeterX = clampX(
                parseInt(newtonMeter.style.left || meterInitialX, 10),
                newtonMeter
            );
            const currentBlockX = clampX(
                parseInt(blockContainer.style.left || blockInitialX, 10),
                block
            );
        ...
        function addRandomness(value, percentRange) {
            const randomFactor =
                1 + (Math.random() * percentRange * 2 - percentRange);
        ...
        function drag(e) {
            ...
            const baseStaticFrictionForce =
                baseKineticFrictionForce * staticFrictionRatio;
            ...
            if (isMoving) {
                ...
                newBlockX =
                    blockInitialX -
                    (pullDistance - (staticFrictionForce * 20 * 5) / maxPull);
        ...
        function updateString() {
            ...
            const meterRight =
                (parseInt(newtonMeter.style.left) || meterInitialX) + 180;
